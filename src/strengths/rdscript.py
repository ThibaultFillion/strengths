...
class RDScript :
    ...
    * units_system : UnitsSystem()
    ...
    def __init__(self,
            ...
            self.init_state_processing = init_state_processing
    ...
    @property 
    def init_state_processing(self) :
        """
        Define how the system state is resampled when the simulation is initialized.
        The resampling state is especially important for stochastic method requiring
        integer values. The propetry can be set with a string with one of the following values:
        ...
        * "redist" : For each species, the floored total quaitity of species in the whole system is determined
            from the initial state. The resampled state is generated by redistribution this floored total
            number of species across the different cells of the system. The probability for a molecule of a given species 
            to be placed inside a certain cell is defined as the corresponding initial state value for this species in this cell, 
            normaized by the floored total quantity of this species.    
        * "auto" : (default) The resampling method is chosen automatically based on the simulation.
            Typically, for stochastic methods, "redist" is used, while for deterministic ones,
            "none" is used.
        """

        return self._init_state_processing
    
    @init_state_processing.setter
    def init_state_processing(self, init_state_processing) :
        if not isinstance(init_state_processing, str):
            raise TypeError("init_state_processing must be a string.")
        if not init_state_processing in ["auto", "none", "Poisson", "redist"]:
            raise ValueError("Accepted values for init_state_processing are \"auto\", \"none\", \"Poisson\" and \"redist\".")
        self._init_state_processing = init_state_processing
